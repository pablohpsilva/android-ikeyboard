...
| 0.8 | 2026-07-24 | Added **ADR-17** (sponsor-ratified): documented the as-built repository layout — flat `crates/` workspace, per-crate `README.md`, flat single-concept `src/`, and centralized repo-root `features/`. Reconciles the docs with the code built through Waves 0–3; drove ARCH v0.4. No architectural-mandate or fitness-function change. |
| 0.9 | 2026-07-24 | Added **ADR-18** (sponsor-ratified): Wave 4 lands the `featherkey-core` composition façade with a UniFFI-*ready* surface; `#[uniffi::export]` scaffolding + Kotlin bindgen deferred to Wave 5 (workspace forbids `unsafe`; bindgen needs the NDK). E-2 sensitive-context ordering (BR-26) enforced + property-tested at the composition root. Notes the additive `Clone` on `dictionary::Dictionary`. No architectural-mandate or fitness-function change. |
| 0.10 | 2026-07-26 | Added **ADR-20** (monorepo restructure: `apps/` deployables + `core/` Rust workspace), **superseding ADR-17**; reserved ADR-19 for the UniFFI `unsafe` carve-out. Path pointers throughout updated to `core/…` / `apps/android/…`. No architectural-mandate or fitness-function change. |
| 0.11 | 2026-08-01 | Amended **ADR-3**: the v1.x neural LM is delivered via the dependency-free `featherkey-nn` micro-net path (neural roadmap apps #1–4), superseding the originally-envisioned `neural-runtime` mechanism; answers open question **Q3** (§16, neural-LM footprint envelope, ≈0.4 MB/user). §15 BR-10/BR-11 traceability rows now also name `featherkey-neural-lm`. No architectural-mandate or fitness-function change. |
| 0.12 | 2026-08-01 | `featherkey-neural-lm` is now wired into the live suggestion strip (neural-LM strip-integration sub-project, SP2): `featherkey-core` owns `NextWordLm`/`RecentWords`, fills the confidence-gated `lm_logprob` 9th feature slot of `featherkey-neural-ranker`, seeds LM candidates at word boundaries, and trains the LM online in `learn_word` (BR-22/26 gated). §15 BR-10/BR-11 traceability rows now also name `featherkey-core`. No exported UniFFI signature changed (bindings-diff-clean verified). No architectural-mandate or fitness-function change. |
...
**Amendment (2026-08-01):** The `neural-runtime` mechanism named in the Decision above never materialized. The v1.x neural LM (BR-10/BR-11) is delivered instead via the dependency-free `featherkey-nn` micro-net path built across the neural roadmap — apps #1–4: `featherkey-neural-ranker` (candidate re-ranking), `featherkey-neural-autocorrect` (the autocorrect-apply gate), `featherkey-neural-tap` (per-user tap-coordinate warp), and `featherkey-neural-lm` (the embedding next-word LM) — superseding `neural-runtime` as the delivery mechanism; see `docs/superpowers/specs/2026-08-01-neural-lm-foundation-design.md` §10. ADR-3's **intent** is honored unchanged: a pluggable neural engine sits behind the stable `prediction` seam, footprint-bounded, fully offline, with no change to callers when it plugs in. Only the **mechanism** changes, and strictly for the better: a tiny, dependency-free net on `featherkey-nn` beats a heavy bundled inference runtime on both the footprint budget (EP-2) and the no-network principle (EP-1), since it adds zero new dependencies. This amendment also **answers open question Q3** (§16, neural-LM footprint envelope): `featherkey-neural-lm`'s fixed-shape model (`k=2` context words, `D=16` embedding dim, `H=32` hidden units, `N=2000`-word per-user vocabulary ceiling) has a bounded, constant footprint of ≈0.4 MB per user (~99,000 `f32` parameters) and an inference latency well under 1 ms, fired only at a word boundary — not a figure it grows into (design doc §8).
...
### ADR-17 — As-built repository layout: flat `crates/` workspace + centralized `features/`
**Status:** **Superseded by ADR-20** — the repo moved to an `apps/` + `core/` monorepo (the Rust workspace now lives under `core/`). The text below is retained as the historical record of the flat repo-root layout used through Waves 0–6b.
**Decision:** The Rust core lives in a single Cargo workspace under **`crates/`** (not a `core/` subfolder); each crate carries its own **`README.md`** (its one job, ports, invariants) and keeps a **flat, single-concept `src/`** (logic may sit directly in `lib.rs` for a small crate, splitting into more files — not mandatory `domain/`/`ports/` subfolders — only as it grows). All BDD **`.feature`** files are **centralized** in the repo-root **`features/`** directory, one `<module>.feature` per crate. Port traits are **not** re-declared per crate: they all live in `contracts` (ADR-12), so no crate owns a `ports/` directory.
**Why:** This is what was actually built through Waves 0–3, and it serves the mandates directly: a single workspace makes the dependency DAG and layer rules machine-checkable in one place (`tools/fitness`, ARCH §13); a centralized `features/` tree lets the BR↔scenario traceability gate (`tools/bdd_check.py`, ARCH §8.3) scan one directory; and the anti-god-file guarantee (AM-5) is enforced by the §6.1 **size/complexity caps**, not by a prescribed folder count — so forcing empty `domain/`/`ports/` scaffolds onto small single-concept crates would add ceremony without adding safety. Recorded as an ADR because it refines the layout described in ARCH §5.2/§11.
**Alternatives:** Restructure the code to the originally-drafted `core/` + per-module `features/` + `domain//ports/` anatomy (large mechanical churn across every crate and a re-verify of all gates, for a purely cosmetic gain, violating principle precedence §1.1); leave the docs describing a layout the repo does not use (doc/reality drift — the failure this ADR closes).
**Consequences:** ARCH bumped to v0.4 to match (§5.2 anatomy, §5.4 registry adds `contracts`, §11 layout). The mandates (AM-1..AM-7) and every fitness function are unchanged — this is an organizational reconciliation, not an architectural change. Modules still unbuilt (`gesture`, `clipboard-core`, `featherkey-core`, `neural-runtime`, `dictation`) adopt this same as-built anatomy when implemented.

### ADR-18 — UniFFI surface authored FFI-ready in Wave 4; export scaffolding + bindgen deferred to Wave 5
**Decision:** `featherkey-core` (Wave 4) presents the shell-facing use-case API in a **UniFFI-ready** shape now — owned plain types (`String`, `f32`, `bool`, flat structs/enums) cross the boundary and every fallible call returns the single flat `FeatherKeyError` (with a `Display` message) — but the actual `#[uniffi::export]` scaffolding, `cdylib` crate-type, and Kotlin-binding generation are applied in **Wave 5**, alongside `ffi-bridge`.
**Why:** Two hard constraints make landing working UniFFI scaffolding inside the Wave-4 workspace crate impossible today, not merely inconvenient. (1) The workspace sets `unsafe_code = "forbid"` for every member (SEDD §5.5), and UniFFI's *generated* scaffolding contains `unsafe` FFI shims — a member crate cannot both forbid unsafe and host the scaffolding without a per-crate lint carve-out that belongs with the seam, not the pure composition logic. (2) Binding generation and the `cdylib` link step target the Android toolchain (NDK), which is absent from the Rust-only sandbox — so the generated Kotlin could not be compiled or verified here regardless. Authoring the surface FFI-shaped now (owned types, no lifetimes/generics across the boundary, one error enum) means Wave 5 **annotates** the existing API rather than redesigning it, and the composition logic + the E-2 gate are fully verified on the host today.
**Alternatives:** Pull `uniffi` in now and carve out `unsafe` for `featherkey-core` (imports the seam's unsafe surface into the pure composition crate and still cannot generate/verify bindings without the NDK — cost now, no verifiable gain); invent a bespoke hand-written JNI surface in Wave 4 (duplicates what UniFFI generates, against SEDD §3.6's UniFFI choice); ship no core-facing API shape until Wave 5 (loses the design/verification the façade provides and couples the Rust API design to the shell schedule).
**Consequences:** The `featherkey-core` public methods are the frozen UniFFI contract; Wave 5 adds the `#[uniffi::export]` annotations, the `cdylib` target, and the `ffi-bridge` marshalling, and validates the generated bindings compile against Kotlin under the `android-shell` CI job (IMPLEMENTATION_PLAN §8). No architectural mandate (AM-1..AM-7) or fitness function changes. A minor, purely-additive `Clone` derive was added to `dictionary::Dictionary` to let the composition root hand owned lexicon copies to the by-value engines — its read-only/immutability invariant is unchanged (no `&mut self` methods).

### ADR-20 — Monorepo restructure: `apps/` (deployables) + `core/` (Rust engine)
**Decision:** The repository is organized as a monorepo. Deployable apps live under **`apps/`** — `apps/android/` today, `apps/web/` reserved for the website — and the Rust engine is a single Cargo workspace under **`core/`** (`core/crates/*`, `core/features/`, `core/tools/`, `core/Cargo.toml`, `core/deny.toml`, `core/rust-toolchain.toml`). Supersedes **ADR-17** (flat repo-root `crates/`).
**Why:** ADR-17 chose a flat repo-root `crates/` layout because the tree then held only the Rust core plus a scaffold Android shell, so a `core/` prefix was pure ceremony. Adding a **web app** alongside the mobile app changes that calculus: `apps/` gives every deployable a symmetric home, and `core/` names the shared engine that both the Android surface (via UniFFI/JNI) and a future web surface (via WASM) consume. Git recorded every move as a rename, so `git log --follow` history is intact; the `__file__`-relative fitness/BDD tooling moved together with `crates/`+`features/` under `core/` and needed no code change.
**Alternatives:** Keep the flat layout and drop the web app into a bare `web/` at the repo root beside `crates/` (asymmetric — mobile among the crates, web on its own; no named shared-engine boundary); adopt heavyweight monorepo tooling (Nx/Turborepo/pnpm) now (YAGNI — there is no JS in the tree until the website lands, and it will introduce its own workspace then).
**Consequences:** `.gitignore`, `apps/android/ffi-bridge/build-jni.sh`, `.github/workflows/ci.yml` (Rust jobs run in `core/`; the committed-native-binary guard stays at the repo root; the Android job runs in `apps/android`), and `core/tools/bdd_check.py` (now locates the BRD at the repo root) were path-fixed and re-verified green — `cargo check --workspace`, `core/tools/fitness/check.py`, `core/tools/bdd_check.py`, and the `:keyboard-view`/`:ime-service` unit tests all pass; `build-jni.sh` resolves to `core/crates/featherkey-core`. ARCH §5.2/§11 were updated to the new tree. No architectural mandate (AM-1..AM-7) or fitness function changed. Play-publishing details live in `PLAY_STORE_PUBLISHING.md`.

### ADR-21 — Correction policy lives in `autocorrect`; the `AutoCorrect` port carries device hints
**Decision:** The whole correction decision — no-clobber validity, candidate gathering, bundled-frequency ordering, momentum scoring, the sticky-fix prior — lives in **`featherkey-autocorrect`**. `featherkey-core::choose_correction` is a delegate that hands the crate a snapshot of the current substrate (every active `LexiconPack` = lexicon + bundled ranks, the learned vocabulary, the active-language set, the live `Momentum`). The port gains a per-call parameter: `fn correct(&self, token: &Token, ctx: &TypingContext, device: &DeviceHints) -> Correction`, where `DeviceHints { known, candidates }` is the platform spell-checker's verdict and candidates. Scoring substrate stays constructor-injected, so the port still describes *a decision*, not *a scoring model*.
**Why:** The system had grown **two** correction engines. The live one (`choose_correction`) sat in the composition root because the port had nowhere to carry what only the shell knows — the device dictionary — while the port-based one (`NoClobberCorrector`) kept the primary lexicon only, no momentum, and alphabetically-ordered candidates. They drifted: the 2026-07-29 bundled-frequency fix landed on one side only. SEDD §5/§15 and ARCH §4/§5.4 name `autocorrect` as the owner of BR-12, BR-15 and BR-45 — the latter two unbuilt — so deleting the crate would have relocated two future requirements into the composition root and falsified the module registry. Moving the policy into its documented home resolves the duplication in the direction the architecture already describes.
**Alternatives:** Delete the crate and re-home BR-12/15/45 on `featherkey-core` (rejected: rewrites the module registry and the SOLID examples to accommodate an accident of implementation order); widen `TypingContext` instead of adding `DeviceHints` (rejected: `TypingContext` is shared with `Predictor`, which has no use for device-dictionary fields); keep the narrow port and add a rich inherent method (rejected: the port impl would then have no caller — dead code in an architectural costume).
**Consequences:** `autocorrect` gains two same-layer dependencies (`candidate-ranker`, `language-momentum`; `domain → domain`, legal under the fitness `LAYER_RANK` rule and acyclic). `LexiconPack` lives in `autocorrect`, not `contracts`, because it holds a `Dictionary` and a port-layer crate may not depend on a domain crate. `FeatherKeyCore::correct` is deleted; the exported FFI `correct` is retained **unchanged in signature** as a thin alias, because the committed UniFFI bindings guard it with a load-time checksum and cannot be regenerated without network access — removing it (and its Kotlin binding) is a follow-up for an environment that can run `uniffi-bindgen`. BR-15 (adjustable aggressiveness) and BR-45 (alternative-word) now have a domain crate to extend. No BR ownership row in §15 changes — which is the point.


> **ADR status:** ADR-1/2/3, **ADR-12–18**, **ADR-20**, and **ADR-21** are **ratified** (sponsor-approved). ADR-19 is **reserved** for the UniFFI `unsafe` carve-out applied with the Wave-5 overlay (an ADR-18 consequence). ADR-4–11 are **proposed defaults** — recorded here with rationale for engineering review; each can be revisited with data before implementation locks it in.
...
| BR-10 | Relevant autocomplete/prediction (*competitive* at MVP; beating iOS prediction quality is a v1.x goal via the neural LM — ADR-3) | `prediction`, `dictionary`, `featherkey-neural-lm`, `featherkey-core` (wires the LM's confidence-gated `lm_logprob` feature + word-boundary candidate seeding into the live strip, SP2) | prediction-quality eval | MVP |
| BR-11 | Prediction improves with use | `prediction`, `personalization`, `neural-runtime`, `featherkey-neural-lm`, `featherkey-core` (online-trains the LM in `learn_word`, BR-22/26 gated, SP2) | longitudinal eval | v1.x |
...
| Q3 | Neural LM footprint envelope | The size/quality trade for the v1.x model; must fit §8.2 | quantization level, model size cap | **Answered** (2026-08-01, ADR-3 amendment): `featherkey-neural-lm` fixed-shape model, ≈0.4 MB/user, <1 ms inference — see `docs/superpowers/specs/2026-08-01-neural-lm-foundation-design.md` §8 |
