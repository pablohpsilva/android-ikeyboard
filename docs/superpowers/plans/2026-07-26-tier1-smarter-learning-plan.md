...
- [ ] **Step 2b: Carry bundled-word frequency into the core (DECISION: option A).** The Rust `Dictionary` is a byte-sorted `fst` with **no frequency**; today `build_packs` (`lib.rs:274`) drops it. Change the language-activation contract so `words` arrive in **frequency order** (most-common first — the shell's asset line order), and `build_packs` records a per-pack `rank: HashMap<String,u32>` (rank = input position) **before** byte-sorting for `Dictionary::from_sorted_words`. Change `packs` to `Vec<(LangId, Dictionary, HashMap<String,u32>)>` (or a small `Pack` struct). Add a core test: a pack built from `["the","cat","aardvark"]` (freq order) yields `rank["the"]==0 < rank["aardvark"]==2` even though the fst stores them alphabetically. Existing callers passing sorted words still work (sorting sorted is a no-op; rank becomes alphabetical, which no existing test checks).
- [ ] **Step 3: Own the whole blend in a new `rank_suggestions` (failing test).** Add `FeatherKeyCore::rank_suggestions(preceding, prefix, device: Vec<Candidate>) -> Vec<RankedCandidate>` that: builds `StatisticalPredictor::new_ranked` over `self.packs` (now `(LangId, Dictionary, rank)`), passing **borrowed** snapshots: the pack `rank` map as `dict_rank`, `self.personalization.frequencies()` (W0 step 6) as learned, and `self.context.next_counts(preceding)` as context; do NOT deep-clone the whole maps per call. Collect the predictor's lang-tagged `Vec<Candidate>`, append `device`, run `featherkey_candidate_ranker::rank(&all, &self.momentum, k)`, then apply the dictionary fold-group variant guarantee. Keep the old `suggest` (`lib.rs:249`) as-is for compatibility. Core tests: typing `hell` yields `he'll`; a commoner bundled word outranks a rarer one at equal context/learned (proves `dict_rank` flows); an `en`-momentum tie promotes the `en` candidate (proves lang survived).
- [ ] **Step 4: Run.** PASS.
...
- [ ] **Step 3b: Pass lexicons in frequency order (frequency-carry, option A).** Where the shell loads `assets/lexicons/<tag>.txt` and activates languages (`FeatherKeyImeService.kt:798-807`), stop byte-sorting before handing words to the core — pass them in **asset (frequency) order** so `build_packs` (W4 Step 2b) records the real rank. (Kotlin keeps its own `Vocabulary`/`rankOf` for swipe, which is unaffected.)
